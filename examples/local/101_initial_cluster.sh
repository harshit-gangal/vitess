...
# start vttablets for keyspace wordpress
for i in 100 101; do
	CELL=zone1 TABLET_UID=$i ./scripts/mysqlctl-up.sh
	CELL=zone1 KEYSPACE=wordpress TABLET_UID=$i ./scripts/vttablet-up.sh
done

# set one of the replicas to master
vtctlclient InitShardMaster -force wordpress/0 zone1-100
